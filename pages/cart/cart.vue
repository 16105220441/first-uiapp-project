<script setup>
	...
	async function updateCartCount() {
		try {
			...
			const {
				data: cartTotalNum
			} = await request(`/cartDetail/get/cartProTotalNum?customerId=${userId}`, "GET");
...
<template>
    ...
    cartListData.list.length  <= 0">
		...
		<view class="cart-list"  v-if="cartListData.list.length  > 0">
			<scroll-view style="height: 100%;" scroll-y="true" @scroll="debouncedHandleScroll">


				<uni-swipe-action>

					<uni-swipe-action-item v-for="(item,index) in cartListData.list" :key="item.id"
						class="bottom-margin">
						<uni-row class="cart-item">

							<view class="check-box">
								<u-checkbox ref="checkBoxRef" :usedAlone="true"
									:checked="!checkGroupBoolean[index].checkBoxBoolean" @change="checkBox(index)"
									activeColor="#c21401" shape="circle"></u-checkbox>
							</view>

							<image :src="item.products?.productImage" alt="" class="product-preview" mode="widthFix"
								srcset="" @click="goToProDetail(item.productId)" />
							<view class="right">
								<view class="details">
									<u-text :lines="2" :text="item.products?.name"
										@click="goToProDetail(item.productId)"></u-text>
									...
									<view class="bottom" style="margin-top: 10px">
										...
										<view class="count">
											<count-box :maxNum="item.products?.stockTotal" v-model:value="item.quantity"
	...
	,item.detailId,index)">
		...
		<view class="page-fix-bottom" v-if=" cartListData.list.length  > 0">
			...
			<view class="left-container">
				<radio :checked="radioCheckBoolean" activeBackgroundColor="#c21401" @click="changeRadio"></radio>
				<text>全选</text>
